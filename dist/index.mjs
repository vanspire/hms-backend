...
var AdminRepository = class {
  ...
  async getDashboardStats() {
    ...
    const [
      ...
      prisma.user.count({ where: { role: Role3.DOCTOR, isActive: true } }),
      prisma.user.count({ where: { role: Role3.PATIENT } }),
...
var UpdateAdminDto = z2.object({
  name: z2.string().optional(),
  phone: z2.string().optional(),
  permissions: z2.array(z2.string()).optional(),
  isActive: z2.boolean().optional()
});
var CreateMedicineDto = z2.object({
  medicineName: z2.string().min(1),
  brand: z2.string().min(1)
});
var UpdateMedicineDto = z2.object({
  medicineName: z2.string().min(1).optional(),
  brand: z2.string().min(1).optional(),
  isActive: z2.boolean().optional()
});
...
router2.get("/dashboard", authenticate, requireRoles([Role5.ADMIN, Role5.SUPERADMIN]), controller2.getDashboard);
router2.get("/medicines", authenticate, requireRoles([Role5.ADMIN, Role5.SUPERADMIN]), controller2.getMedicines);
router2.post("/medicines", authenticate, requireRoles([Role5.ADMIN, Role5.SUPERADMIN]), controller2.createMedicine);
router2.put("/medicines/:id", authenticate, requireRoles([Role5.ADMIN, Role5.SUPERADMIN]), controller2.updateMedicine);
router2.patch("/medicines/:id/deactivate", authenticate, requireRoles([Role5.ADMIN, Role5.SUPERADMIN]), controller2.deactivateMedicine);
router2.use(authenticate, requireRoles([Role5.SUPERADMIN]));
...
var CreateDoctorDto = z3.object({
  name: z3.string().min(1, "Name is required"),
  departmentId: z3.string().uuid("Invalid department ID pattern"),
  phone: z3.string().max(10, "Phone must be max 10 digits").regex(/^\d+$/, "Must be numeric"),
  username: z3.string().min(4, "Username must be at least 4 characters"),
  password: z3.string().min(6, "Password must be at least 6 characters"),
  consultationFee: z3.number().max(1e4, "Fee cannot exceed 10000"),
  incrementIntervalDays: z3.number().max(365, "Interval cannot exceed 365 days"),
  renewalCharge: z3.number().max(5e3, "Renewal charge cannot exceed 5000")
});
var UpdateDoctorDto = z3.object({
  name: z3.string().min(1, "Name is required").optional(),
  departmentId: z3.string().uuid("Invalid department ID pattern").optional(),
  phone: z3.string().max(10, "Phone must be max 10 digits").regex(/^\d+$/, "Must be numeric").optional(),
  consultationFee: z3.number().max(1e4, "Fee cannot exceed 10000").optional(),
  incrementIntervalDays: z3.number().max(365, "Interval cannot exceed 365 days").optional(),
  renewalCharge: z3.number().max(5e3, "Renewal charge cannot exceed 5000").optional()
});
...
router3.post("/", authenticate, requireRoles([Role7.SUPERADMIN, Role7.ADMIN]), controller3.create);
...
var ReceptionistService = class {
  ...
  async createReceptionist(data) {
    ...
    return this.repository.createReceptionist(
      { username: data.username, passwordHash, role: Role8.RECEPTIONIST, isActive: true },
...
var CreateReceptionistDto = z4.object({
  name: z4.string().min(1, "Name is required"),
  phone: z4.string().max(10, "Phone must be max 10 digits").regex(/^\d+$/, "Must be numeric"),
  username: z4.string().min(4, "Username must be at least 4 characters"),
...
router4.post("/", authenticate, requireRoles([Role9.SUPERADMIN, Role9.ADMIN]), controller4.create);
...
var RegisterPatientDto = z5.object({
  name: z5.string().min(1, "Name is required"),
  firstName: z5.string().optional(),
  lastName: z5.string().optional(),
  age: z5.number().int().nonnegative("Age cannot be negative"),
  dob: z5.string().optional(),
  gender: z5.string().optional(),
  // Structured Address
  houseNumber: z5.string().optional(),
  houseName: z5.string().optional(),
  houseAddress: z5.string().optional(),
  localArea: z5.string().optional(),
  street: z5.string().optional(),
  city: z5.string().optional(),
  state: z5.string().optional(),
  pincode: z5.string().optional(),
  // Contact info
  contactNo: z5.string().optional(),
  alternateContact: z5.string().optional(),
  email: z5.string().email("Invalid email").optional().or(z5.literal("")),
  address: z5.string().optional(),
  // Legacy combined
  // Guardian Info
  guardianName: z5.string().optional(),
  guardianRelation: z5.string().optional(),
  guardianContact: z5.string().optional(),
  guardianComment: z5.string().optional(),
  guardianAddress: z5.string().optional(),
  // Referral
  referredDoctor: z5.string().optional(),
  referredHospital: z5.string().optional(),
  // ID Proof
  idProofType: z5.string().optional(),
  idProofDetail: z5.string().optional(),
  idProofData: z5.string().optional(),
  registrationAmount: z5.number().optional().default(200),
  registrationDate: z5.string().optional()
  // ISO date
});
var UpsertPatientMedicalHistoryDto = z5.object({
  pastMedicalHistory: z5.string().optional(),
  pastSurgicalHistory: z5.string().optional(),
  allergies: z5.string().optional(),
  familyHistory: z5.string().optional(),
  socialHistory: z5.string().optional(),
  medications: z5.string().optional(),
  vaccinationHistory: z5.string().optional()
});
...
import { PaymentMode as PaymentMode3 } from "@prisma/client";

// src/modules/booking/booking.dto.ts
import { z as z6 } from "zod";
var CreateSlotDto = z6.object({
  doctorId: z6.string().uuid(),
  startTime: z6.string().datetime(),
  endTime: z6.string().datetime(),
  maxCapacity: z6.number().int().positive().default(1)
});
var CreateBulkSlotsDto = z6.object({
  doctorId: z6.string().uuid(),
  startDate: z6.string().datetime().optional(),
  endDate: z6.string().datetime().optional(),
  startTimeStr: z6.string().regex(/^([01]\d|2[0-3]):?([0-5]\d)$/),
  endTimeStr: z6.string().regex(/^([01]\d|2[0-3]):?([0-5]\d)$/),
  slotDurationMinutes: z6.number().int().positive().default(15),
  breakDurationMinutes: z6.number().int().min(0).default(0),
  maxCapacity: z6.number().int().positive().default(1),
  daysOfWeek: z6.array(z6.number().int().min(0).max(6)).optional(),
  specificDates: z6.array(z6.string().datetime()).optional()
}).refine(
...
var BookAppointmentDto = z6.object({
  patientId: z6.string().uuid(),
  doctorId: z6.string().uuid(),
  slotId: z6.string().uuid(),
  paymentMode: z6.enum(["CASH", "CREDIT", "UPI", "STRIPE"])
});
var UpdateVisitDto = z6.object({
  chiefComplaint: z6.string().optional(),
  examination: z6.string().optional(),
  diagnosis: z6.string().optional(),
  diagnosisIcd10: z6.string().optional(),
  treatmentPlan: z6.string().optional(),
  consultationNotes: z6.string().optional(),
  notes: z6.string().optional(),
  medications: z6.array(z6.object({
    medicineId: z6.string().uuid(),
    brand: z6.string().min(1),
    drug: z6.string().min(1),
    strength: z6.string().min(1),
    dosageForm: z6.string().min(1),
    dose: z6.string().min(1),
    unit: z6.string().min(1),
    frequency: z6.string().min(1),
    duration: z6.string().min(1),
    durationUnit: z6.string().min(1),
    instructions: z6.string().min(1),
    notes: z6.string().optional()
  })).optional(),
  labRequests: z6.array(z6.object({
    labTestId: z6.string().uuid().optional(),
    testName: z6.string().min(1).optional(),
    clinicalNotes: z6.string().optional(),
    status: z6.enum(["REQUESTED", "SAMPLE_COLLECTED", "REPORT_UPLOADED", "COMPLETED"]).default("REQUESTED")
  })).optional(),
  radiologyRequests: z6.array(z6.object({
    radiologyTestId: z6.string().uuid().optional(),
    modality: z6.enum(["X_RAY", "MRI", "CT_SCAN", "ULTRASOUND"]).optional(),
    testName: z6.string().min(1).optional(),
    clinicalNotes: z6.string().optional(),
    status: z6.enum(["REQUESTED", "SCHEDULED", "REPORT_UPLOADED", "COMPLETED"]).default("REQUESTED")
  })).optional(),
  vitals: z6.object({
    temperature: z6.string().optional(),
    pulse: z6.string().optional(),
    spo2: z6.string().optional(),
    bpSystolic: z6.string().optional(),
    bpDiastolic: z6.string().optional(),
    respiratoryRate: z6.string().optional(),
    nurseNotes: z6.string().optional()
  }).optional()
});

// src/modules/booking/booking.service.ts
import { PaymentStatus as PaymentStatus3 } from "@prisma/client";
...
var InvoiceService = class {
  ...
  async generateInvoice(appointmentId, payload) {
    ...
    let status = payload.status ?? InvoiceStatus.ISSUED;
...
import { AppointmentStatus, BillingCategory, LabRequestStatus, PaymentStatus as PaymentStatus2, RadiologyRequestStatus } from "@prisma/client";
    ...
    await prisma.$transaction(async (tx) => {
      ...
      for (const request of data.labRequests ?? []) {
        ...
        if (existing) {
          ...
          await tx.labRequest.create({ data: { visitId: visit.id, appointmentId, patientId: appointment.patientId, doctorId: appointment.doctorId, labTestId: catalog.id, testNameSnapshot: catalog.testName, priceSnapshot: catalog.price, clinicalNotes: request.clinicalNotes, status: request.status ?? LabRequestStatus.REQUESTED } });
...
var BookingService = class {
  ...
  async payAppointment(appointmentId, amount, paymentMode) {
    ...
    const newStatus = newPending === 0 ? PaymentStatus3.PAID : newPaid > 0 ? PaymentStatus3.PARTIAL : PaymentStatus3.PENDING;
  ...
  payAppointment = async (req, res) => {
    try {
      const result = await this.service.payAppointment(
        ...
        req.body.paymentMode || PaymentMode3.CASH
      );
...
router6.post("/slots", authenticate, requireRoles([Role10.SUPERADMIN, Role10.ADMIN, Role10.RECEPTIONIST]), controller6.createSlot);
router6.post("/slots/bulk", authenticate, requireRoles([Role10.SUPERADMIN, Role10.ADMIN, Role10.RECEPTIONIST]), controller6.createBulkSlots);
...
router6.post("/book", authenticate, requireRoles([Role10.RECEPTIONIST, Role10.PATIENT, Role10.ADMIN, Role10.SUPERADMIN]), controller6.bookAppointment);
...
router6.put("/appointments/:id/visit", authenticate, requireRoles([Role10.SUPERADMIN, Role10.DOCTOR]), controller6.updateVisit);
router6.patch("/appointments/:id/complete", authenticate, requireRoles([Role10.SUPERADMIN, Role10.DOCTOR]), controller6.completeAppointment);
...
var UpdateOrganizationDto = z7.object({
  organizationName: z7.string().optional(),
  timezone: z7.string().optional(),
  currency: z7.string().optional(),
  contactEmail: z7.string().email().optional().or(z7.literal("")),
...
router7.get("/organization", requireRoles([Role11.SUPERADMIN, Role11.ADMIN, Role11.DOCTOR, Role11.RECEPTIONIST]), controller7.getOrganization);
router7.put("/organization", requireRoles([Role11.SUPERADMIN, Role11.ADMIN]), controller7.updateOrganization);
...
var CreateMedicalReportDto = z8.object({
  patientId: z8.string().uuid(),
  reportType: z8.string().min(2),
  description: z8.string().optional(),
  fileUrl: z8.string().url()
});
...
router8.post("/", authenticate, requireRoles([Role12.SUPERADMIN, Role12.ADMIN, Role12.DOCTOR, Role12.RECEPTIONIST, Role12.PATIENT]), controller8.create);
...
var CreateDepartmentDto = z9.object({
  name: z9.string().min(2, "Name must be at least 2 characters"),
  description: z9.string().optional()
});
var UpdateDepartmentDto = z9.object({
  name: z9.string().min(2, "Name must be at least 2 characters").optional(),
  description: z9.string().optional()
});
...
router9.use(requireRoles([Role13.ADMIN, Role13.SUPERADMIN]));
...
var CreateLabTestDto = z11.object({
  testName: z11.string().min(1),
  testCode: z11.string().min(1),
  price: z11.number().positive(),
  description: z11.string().optional(),
  department: z11.string().optional()
});
var UpdateLabTestDto = z11.object({
  testName: z11.string().min(1).optional(),
  testCode: z11.string().min(1).optional(),
  price: z11.number().positive().optional(),
  description: z11.string().optional(),
  department: z11.string().optional(),
  isActive: z11.boolean().optional()
});
var CreateLabRequestDto = z11.object({
  visitId: z11.string().uuid(),
  appointmentId: z11.string().uuid(),
  patientId: z11.string().uuid(),
  doctorId: z11.string().uuid(),
  labTestId: z11.string().uuid(),
  clinicalNotes: z11.string().optional()
});
var UpdateLabRequestStatusDto = z11.object({
  status: z11.enum(["REQUESTED", "SAMPLE_COLLECTED", "REPORT_UPLOADED", "COMPLETED"])
});
var CreateLabReportDto = z11.object({
  labRequestId: z11.string().uuid(),
  fileUrl: z11.string().url(),
  uploadedBy: z11.string().min(1),
  reportNotes: z11.string().optional()
});

// src/modules/lab/lab.repository.ts
import { BillingCategory as BillingCategory2, LabRequestStatus as LabRequestStatus2 } from "@prisma/client";
var LabRepository = class {
  ...
  async createRequest(data) {
    return prisma.$transaction(async (tx) => {
      ...
      await tx.billingItem.create({ data: { appointmentId: data.appointmentId, category: BillingCategory2.LAB_TEST, itemId: data.labTestId, itemName: data.testNameSnapshot, quantity: 1, unitPrice: data.priceSnapshot, totalPrice: data.priceSnapshot } });
  ...
  async createReport(data) {
    return prisma.$transaction(async (tx) => {
      ...
      await tx.labRequest.update({ where: { id: data.labRequestId }, data: { status: LabRequestStatus2.REPORT_UPLOADED } });
      return report;
    });
  }
  getReportById(id) {
...
router10.post("/catalog", authenticate, requireRoles([Role14.ADMIN, Role14.SUPERADMIN]), controller10.createTest);
router10.put("/catalog/:id", authenticate, requireRoles([Role14.ADMIN, Role14.SUPERADMIN]), controller10.updateTest);
router10.patch("/catalog/:id/deactivate", authenticate, requireRoles([Role14.ADMIN, Role14.SUPERADMIN]), controller10.deactivateTest);
...
router10.post("/requests", authenticate, requireRoles([Role14.DOCTOR, Role14.SUPERADMIN]), controller10.createRequest);
router10.patch("/requests/:id/status", authenticate, requireRoles([Role14.ADMIN, Role14.SUPERADMIN]), controller10.updateRequestStatus);
router10.post("/reports", authenticate, requireRoles([Role14.ADMIN, Role14.SUPERADMIN]), controller10.createReport);
router10.post("/requests/:id/report", authenticate, requireRoles([Role14.ADMIN, Role14.SUPERADMIN]), controller10.createReport);
...
import { Role as Role15 } from "@prisma/client";

// src/modules/radiology/radiology.dto.ts
import { z as z12 } from "zod";
var CreateRadiologyTestDto = z12.object({
  testName: z12.string().min(1),
  modality: z12.enum(["X_RAY", "MRI", "CT_SCAN", "ULTRASOUND"]),
  price: z12.number().positive(),
  description: z12.string().optional()
});
var UpdateRadiologyTestDto = z12.object({
  testName: z12.string().min(1).optional(),
  modality: z12.enum(["X_RAY", "MRI", "CT_SCAN", "ULTRASOUND"]).optional(),
  price: z12.number().positive().optional(),
  description: z12.string().optional(),
  isActive: z12.boolean().optional()
});
var CreateRadiologyRequestDto = z12.object({
  visitId: z12.string().uuid(),
  appointmentId: z12.string().uuid(),
  patientId: z12.string().uuid(),
  doctorId: z12.string().uuid(),
  radiologyTestId: z12.string().uuid(),
  clinicalNotes: z12.string().optional()
});
var UpdateRadiologyRequestStatusDto = z12.object({
  status: z12.enum(["REQUESTED", "SCHEDULED", "REPORT_UPLOADED", "COMPLETED"])
});
var CreateRadiologyReportDto = z12.object({
  radiologyRequestId: z12.string().uuid(),
  fileUrl: z12.string().url(),
  uploadedBy: z12.string().min(1),
  reportNotes: z12.string().optional()
});

// src/modules/radiology/radiology.repository.ts
import { BillingCategory as BillingCategory3, RadiologyRequestStatus as RadiologyRequestStatus2 } from "@prisma/client";
var RadiologyRepository = class {
  ...
  async createRequest(data) {
    return prisma.$transaction(async (tx) => {
      ...
      await tx.billingItem.create({ data: { appointmentId: data.appointmentId, category: BillingCategory3.RADIOLOGY_TEST, itemId: data.radiologyTestId, itemName: data.testNameSnapshot, quantity: 1, unitPrice: data.priceSnapshot, totalPrice: data.priceSnapshot } });
      await tx.appointment.update({ where: { id: data.appointmentId }, data: { totalAmount: { increment: data.priceSnapshot }, pendingAmount: { increment: data.priceSnapshot } } });
      return request;
    });
  }
  ...
  async createReport(data) {
    return prisma.$transaction(async (tx) => {
      ...
      await tx.radiologyRequest.update({ where: { id: data.radiologyRequestId }, data: { status: RadiologyRequestStatus2.REPORT_UPLOADED } });
      return report;
    });
  }
  getReportById(id) {
...
router11.post("/catalog", authenticate, requireRoles([Role15.ADMIN, Role15.SUPERADMIN]), controller11.createTest);
router11.put("/catalog/:id", authenticate, requireRoles([Role15.ADMIN, Role15.SUPERADMIN]), controller11.updateTest);
router11.patch("/catalog/:id/deactivate", authenticate, requireRoles([Role15.ADMIN, Role15.SUPERADMIN]), controller11.deactivateTest);
...
router11.post("/requests", authenticate, requireRoles([Role15.DOCTOR, Role15.SUPERADMIN]), controller11.createRequest);
router11.patch("/requests/:id/status", authenticate, requireRoles([Role15.ADMIN, Role15.SUPERADMIN]), controller11.updateRequestStatus);
router11.post("/reports", authenticate, requireRoles([Role15.ADMIN, Role15.SUPERADMIN]), controller11.createReport);
router11.post("/requests/:id/report", authenticate, requireRoles([Role15.ADMIN, Role15.SUPERADMIN]), controller11.createReport);
...
import { Role as Role16 } from "@prisma/client";

// src/modules/billing/billing.dto.ts
import { z as z13 } from "zod";
var CreateBillingItemDto = z13.object({
  appointmentId: z13.string().uuid(),
  category: z13.enum(["CONSULTATION", "REGISTRATION", "LAB_TEST", "RADIOLOGY_TEST", "OTHER"]),
  itemId: z13.string().optional(),
  itemName: z13.string().min(1),
  quantity: z13.number().int().positive(),
  unitPrice: z13.number().nonnegative()
});
var UpdateBillingItemDto = z13.object({
  itemName: z13.string().min(1).optional(),
  quantity: z13.number().int().positive().optional(),
  unitPrice: z13.number().nonnegative().optional()
});
...
router12.post("/", authenticate, requireRoles([Role16.ADMIN, Role16.SUPERADMIN, Role16.RECEPTIONIST]), controller12.createItem);
router12.put("/:id", authenticate, requireRoles([Role16.ADMIN, Role16.SUPERADMIN, Role16.RECEPTIONIST]), controller12.updateItem);
router12.delete("/:id", authenticate, requireRoles([Role16.ADMIN, Role16.SUPERADMIN]), controller12.deleteItem);
var billing_routes_default = router12;

// src/modules/invoice/invoice.routes.ts
import { Router as Router13 } from "express";
import { Role as Role17 } from "@prisma/client";

// src/modules/invoice/invoice.dto.ts
import { InvoiceStatus as InvoiceStatus2 } from "@prisma/client";
import { z as z14 } from "zod";
var GenerateInvoiceDto = z14.object({
  taxRate: z14.number().min(0).max(1).optional(),
  status: z14.nativeEnum(InvoiceStatus2).optional()
});
var UpdateInvoiceStatusDto = z14.object({
  status: z14.nativeEnum(InvoiceStatus2)
});
...
router13.post("/generate/:appointmentId", authenticate, requireRoles([Role17.ADMIN, Role17.SUPERADMIN, Role17.RECEPTIONIST]), controller13.generate);
...
router13.patch("/:id/status", authenticate, requireRoles([Role17.ADMIN, Role17.SUPERADMIN, Role17.RECEPTIONIST]), controller13.updateStatus);
var invoice_routes_default = router13;

// src/modules/analytics/analytics.routes.ts
import { Router as Router14 } from "express";
import { Role as Role18 } from "@prisma/client";
...
var AnalyticsRepository = class {
  async revenueByBucket(unit, start, end) {
    return prisma.$queryRaw(Prisma5.sql`
  ...
  getDepartmentRevenue(start, end) {
    return prisma.$queryRaw(Prisma5.sql`
      ...
      WHERE ${start && end ? Prisma5.sql`bi."createdAt" >= ${start} AND bi."createdAt" <= ${end}` : Prisma5.sql`TRUE`}
  ...
  getPatientRevenue(start, end) {
    return prisma.$queryRaw(Prisma5.sql`
      ...
      WHERE ${start && end ? Prisma5.sql`a."createdAt" >= ${start} AND a."createdAt" <= ${end}` : Prisma5.sql`TRUE`}
...
router14.use(authenticate, requireRoles([Role18.ADMIN, Role18.SUPERADMIN]));
